...
parser.add_argument('-d', dest='directry', required=True)
parser.add_argument('-l', dest='level', required=True)
parser.add_argument('-s', dest='session_id', required=True)
args = parser.parse_args()

ds_ec_raw_dic=pickle.load(open(args.directry+'/ec_domain_'+args.level+'_pickle_'+args.session_id,'rb'))
# data structure
# {ds:[ec1,ec2, ...], ...}
